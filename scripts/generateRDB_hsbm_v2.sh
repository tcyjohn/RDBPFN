...
RAW_OUTPUT_DIR=/data/caijunyu/RDBPFN/data_generation/RDB_datasets/plurel_scale_complex_hsbm_v2
DST_PREFIX="dag_rdb_complex_hsbm_v2_"
...
for db_path in "${HOME}/scratch/relbench/${DST_PREFIX}"*/; do
  db=$(basename "$db_path")
  pixi run python -u -m rt.embed --dataset_name "$db"
done
shopt -u nullglob
...
echo ""
echo "=== Pretrain not included in this script ==="
echo "Run manually with:"
echo "  cd /data/caijunyu/relational-transformer"
echo "  pixi run torchrun --standalone --nproc_per_node=2 scripts/example_pretrain_dag_rdb.py \\"
echo "    --db-prefix \"${DST_PREFIX}\" \\"
echo "    --num-datasets ${HSBM_COUNT}"
