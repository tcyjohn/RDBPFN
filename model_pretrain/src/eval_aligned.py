...
class _AlignedDownsample:
    X: np.ndarray
    y: np.ndarray
    aligned_arrays: tuple[np.ndarray | None, ...]
...
def _downsample_aligned(
    ...
    for array in aligned_arrays:
        if array is not None and len(array) != len(X):
            raise ValueError(
                "Aligned arrays must have the same number of rows as X: "
                f"expected {len(X)}, got {len(array)}"
            )

    if max_samples is None or len(X) <= max_samples:
        return _AlignedDownsample(X, y, aligned_arrays)

    rng = np.random.default_rng(seed)
    indices = rng.choice(len(X), size=max_samples, replace=False)
    sampled_arrays = tuple(
        array[indices] if array is not None else None for array in aligned_arrays
    )
    return _AlignedDownsample(X[indices], y[indices], sampled_arrays)
...
def _evaluate_datasets(
    ...
    use_primary_key_as_entity_id = bool(
        OmegaConf.select(
            cfg, "dataset.use_primary_key_as_entity_id", default=False
        )
    )
    ...
    for dataset_path_str in cfg.dataset.paths:
        ...
        for task in dataset.tasks:
            if task.metadata.task_type != DBBTaskType.classification:
                ...
                continue
            try:
                X_train, y_train, fk_values_train, entity_ids_train, parent_entity_ids_train = load_task_split(
                    task,
                    "train",
                    use_primary_key_as_entity_id=use_primary_key_as_entity_id,
                )
                X_test, y_test, fk_values_test, entity_ids_test, parent_entity_ids_test = load_task_split(
                    task,
                    "test",
                    use_primary_key_as_entity_id=use_primary_key_as_entity_id,
                )
        ...
        for seed in cfg.dataset.seeds:
            for X_train, y_train, X_test, y_test, task, task_name, fk_values_train, entity_ids_train, parent_entity_ids_train, fk_values_test, entity_ids_test, parent_entity_ids_test in task_entries:
                ...
                test_split = _downsample_aligned(
                    X_test,
                    y_test,
                    max_test_samples,
                    test_seed_offset,
                ...
                X_test_ds = test_split.X
                y_test_ds = test_split.y
                (
                    fk_values_test_ds,
                    entity_ids_test_ds,
                    parent_entity_ids_test_ds,
                ) = test_split.aligned_arrays
                ...
                task_result = _evaluate_task(
                    ...
                    fk_values_test=fk_values_test_ds,
                    entity_ids_test=entity_ids_test_ds,
                    parent_entity_ids_test=parent_entity_ids_test_ds,
                )
