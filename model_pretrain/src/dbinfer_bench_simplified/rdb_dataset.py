...
class DBBRDBDataset:
    ...
    def _load_metadata(self):
        return load_pyd(DBBRDBDatasetMeta, self.path / "metadata.yaml")
    ...
    def _load_data(self):
        ...
        for task_meta in self.metadata.tasks:
            ...
            self._tasks.append(
                DBBRDBTask(task_meta, train_set, validation_set, test_set)
            )
...
class DBBRDBTaskCreator:
    ...
    def add_task_data(
        ...
        **extra_meta,
        ...
        assert (
            ...
            or validation_data is not None
            or test_data is not None
        )
    ...
    def done(
        ...
        task_meta["source"] = str(
            table_writer.filename(task_path, "{split}").relative_to(path)
        )
...
class DBBRDBDatasetCreator:
    ...
    def add_column(
        ...
        **extra_meta,
        ...
        if table_name not in self.tables:
            raise ValueError(
                f"Table {table_name} does not exist. Please add_table first."
            )
    ...
    def done(
        ...
        save_pyd(metadata, path / "metadata.yaml")
