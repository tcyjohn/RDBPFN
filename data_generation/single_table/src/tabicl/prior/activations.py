...
class RandomFunctionActivation(nn.Module):
    ...
    def __init__(self, n_frequencies: int = 256):
        ...
        # Generate frequencies in range [0.1, n_frequencies] to avoid zeros
        # Using 0.1 as minimum ensures no division by zero issues
        self.freqs = nn.Parameter(
            0.1 + (n_frequencies - 0.1) * torch.rand(n_frequencies),
            requires_grad=False
        )
        ...
        with torch.no_grad():
            ...
            if torch.any(torch.isnan(freq_factors)) or torch.any(torch.isinf(freq_factors)):
                # Fallback to uniform weights if something goes wrong
                freq_factors = torch.ones(n_frequencies)
...
class RandomScaleLayer(nn.Module):
    ...
    def initialize(self, x: torch.Tensor):
        ...
        self.scale = torch.exp(np.log(1.0) + 1.4 * torch.randn(1, n_out, device=x.device))
...
def get_activations(random: bool = True, scale: bool = True, diverse: bool = True):
    ...
    simple_activations = [
        ...
        # SignActivation,
        RBFActivation,
        # ExpActivation, # This may cause too large values in the output
        SqrtAbsActivation,
