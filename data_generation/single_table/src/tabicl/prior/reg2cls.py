...
class MulticlassAssigner(nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        if self.mode == "rank":
            ...
            if torch.unique(boundaries).shape[0] == 1:
                if boundaries[0] == torch.max(input) or boundaries[0] == torch.min(input):
                    boundaries[0] = torch.mean(input)
    ...
    def __init__(self, hp: dict):
        ...
        if num_classes == 0:
            ...
            self.class_assigner = MulticlassAssigner(
                ...
                num_classes, mode="rank", ordered_prob=self.hp["multiclass_ordered_prob"]
            )
        else:
