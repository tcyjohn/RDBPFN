...
from src.prior.temporal_vocab import EventCalendar
...
class ColumnDataProcessor:
    ...
    def _convert_to_datetime64(self, raw_data: torch.Tensor) -> torch.Tensor:
        """Convert day indices to days-since-epoch.

        raw_data contains day indices [0, NUM_DAYS) generated by the
        calendar-aligned temporal pipeline.  We clamp to the valid range
        and return as int64 days since 1970-01-01.

        Parameters:
        -----------
        raw_data : torch.Tensor
            Day-index tensor in [0, NUM_DAYS).
        ...
        """
        from src.prior.temporal_vocab import NUM_DAYS  # noqa: PLC0415

        max_days = NUM_DAYS - 1

        finite_mask = torch.isfinite(raw_data)
        if not torch.any(finite_mask):
            return torch.full_like(raw_data, max_days // 2, dtype=torch.long)

        finite_data = raw_data[finite_mask]
        median_val = torch.median(finite_data) if len(finite_data) > 0 else torch.tensor(0.0)
        clamped = torch.where(finite_mask, raw_data, median_val.to(raw_data.device))
        clamped = clamped.clamp(0.0, float(max_days))
        return clamped.long()
...
class TableGenerator:
    ...
    def _compute_t_min_for_child(
        ...
        Returns:
            (child_rows,) tensor of t_min values **in days** (0 .. NUM_DAYS).
        """
        from src.prior.temporal_vocab import NUM_DAYS  # noqa: PLC0415
        ...
        # Collect parent timestamps (day indices) for rows with FK references
        ...
        if parent_ts_list:
            ...
            # Fallback based on DAG topology (in days)
...
class RDB:
    ...
    def __init__(self, name: str = "default_rdb"):
        ...
        self._seed: int = 0
        self.event_calendar: EventCalendar | None = None
    ...
    def init_table_SCMs(
        ...
        # Create RDB-level EventCalendar (shared across all timestamp tables)
        ec_seed = hash((seed, "event_calendar")) & 0x7FFFFFFF
        self.event_calendar = EventCalendar(seed=ec_seed, device=self.device)
        ...
        for table_name in nx.topological_sort(self.graph):
            ...
            # Wire EventCalendar to timestamp tables
            if table.is_time_table and self.event_calendar is not None:
                sens_seed = hash((seed, table_name, "table_sens")) & 0x7FFFFFFF
                table_sens = EventCalendar.sample_table_sens(
                    self.event_calendar.H, sens_seed, device=self.device
                )
                tg = self.table_generators[table_name]
                if tg.table_SCM.temporal_vocab is not None:
                    tg.table_SCM.temporal_vocab.set_calendar(
                        self.event_calendar, table_sens
                    )
    ...
    def save_generation_schemas(self, file_path: str) -> None:
        ...
        generation_schemas = {
            "rdb_name": self.name,
            "event_calendar": self.event_calendar.to_dict() if self.event_calendar else None,
