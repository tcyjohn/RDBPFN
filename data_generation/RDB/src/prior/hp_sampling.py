...
class HpSampler(nn.Module):
    ...
    def setup_meta_beta_sampler(self):
        ...
        # Support separate ranges for b and k if b_min/b_max and k_min/k_max are provided
        if hasattr(self, "b_min") and hasattr(self, "b_max"):
            b_min, b_max = self.b_min, self.b_max
        else:
            b_min, b_max = self.min, self.max

        if hasattr(self, "k_min") and hasattr(self, "k_max"):
            k_min, k_max = self.k_min, self.k_max
        else:
            k_min, k_max = self.min, self.max

        self.ensure_hyperparameter("b", "uniform", b_min, b_max)
        self.ensure_hyperparameter("k", "uniform", k_min, k_max)
...
class HpSamplerList(nn.Module):
    ...
    def __init__(self, hyperparameters, device, seed):
        super().__init__()
        np.random.seed(seed)
...
if __name__ == "__main__":
    ...
    sampler = HpSamplerList(DEFAULT_SAMPLED_HP, device="cuda", seed=42)
    params = sampler.sample()
    for k, v in params.items():
        value = v() if callable(v) else v
        print(f"{k}: {value}")

    new_params = sampler.sample()
