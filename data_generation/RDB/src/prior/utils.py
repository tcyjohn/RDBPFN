...
class MASK_TYPE(Enum):
    ...
    TIMESTAMP = "timestamp"
    MASS_EMBEDDING = "mass_embedding"
    AUG_EMBEDDING = "aug_embedding"


class SCM_OUTPUT:
    # Represents the values of the output, shape: (num_samples, total_features)
    values: torch.Tensor
    # Represents the selcted map and their idx
    mask_dict: Dict[MASK_TYPE, torch.Tensor]
...
# TODO: Set more pre-defined distributions for the input features
class XSampler:
    ...
    def sample_batch(self, batch_size: int, return_numpy: bool = False):
        """Generate features for a specific batch size.
        ...
        X : torch.Tensor or numpy.ndarray
            Generated features of shape (batch_size, num_features)
        """
        samplers = {
            "normal": self.sample_normal_all,
            "mixed": self.sample_mixed,
        ...
        # Temporarily change seq_len for this batch
