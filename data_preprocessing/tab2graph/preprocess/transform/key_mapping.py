...
class KeyMapping(RDBTransform):
    ...
    def fit(
        ...
        # If a primary key is not in the relationship, it is a dangling primary key.
        ...
        for tbl_name, tbl in rdb_data.tables.items():
            ...
            for col_name, col in tbl.items():
                if col.metadata['dtype'] == DBBColumnDType.primary_key:
                    if (tbl_name, col_name) not in self.fk_to_pk:
                        self.fk_to_pk[(tbl_name, col_name)] = (tbl_name, col_name)
                        self.pk_to_fk_list[(tbl_name, col_name)] = []
