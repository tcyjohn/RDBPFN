...
from .tabnn import get_tabnn_class, get_tabnn_class_names
...
class TabularDataset(Dataset):
    def __init__(self, table: Dict[str, np.ndarray], features: List[str]):
        self.table = {col: torch.from_numpy(val) for col, val in table.items() if col in features}
        self.columns = [col for col in table.keys() if col in features]
...
class TabNNSolutionConfig(TabularMLSolutionConfig):
    ...
    # NOTE(minjie): When launching jobs to AWS batch, suggest to stick to this default
    #   value to avoid resource contention among multiple jobs when they are
    #   scheduled to the same physical machine.
    use_multiprocessing : bool = True
    max_num_dataloader_workers : int = 12
...
class TabNNSolution(TabularMLSolution):
    ...
    def create_dataloader(
        ...
        feature_list = [feat for feat in self.data_config.features.keys()]
        if mode == "eval" and self.data_config.task.task_type == "retrieval":
            feature_list += [
                self.data_config.task.key_prediction_label_column,
                self.data_config.task.key_prediction_query_idx_column,
            ]
        dataset = TabularDataset(table, feature_list)
        ...
        if self.solution_config.use_multiprocessing:
            num_workers = min(device.cpu_count // 2, self.solution_config.max_num_dataloader_workers)
        else:
            num_workers = 0
...
# Register NN class name as solutions too, making them directly usable in
# commandlines like fit-tab.

class MLPSolutionConfig(TabNNSolutionConfig):
    nn_name : str = "mlp"
    nn_config : pydantic.BaseModel = get_tabnn_class("mlp").config_class()

@tabml_solution
class MLPSolution(TabNNSolution):
    config_class = MLPSolutionConfig
    name = "mlp"

class FTTSolutionConfig(TabNNSolutionConfig):
    nn_name : str = "fttransformer"
    nn_config : pydantic.BaseModel = get_tabnn_class("fttransformer").config_class()

@tabml_solution
class FTTSolution(TabNNSolution):
    config_class = FTTSolutionConfig
    name = "fttransformer"

class DeepFMSolutionConfig(TabNNSolutionConfig):
    nn_name : str = "deepfm"
    nn_config : pydantic.BaseModel = get_tabnn_class("deepfm").config_class()

@tabml_solution
class DeepFMSolution(TabNNSolution):
    config_class = DeepFMSolutionConfig
    name = "deepfm"
